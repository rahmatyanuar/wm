...
<script src="js/xtreecheckbox.js"></script>
</HEAD>
<BODY>
			...
			%loop servers%
             ...
             %ifvar status equals('Success')%
              ...
              %loop assetComposites%		
                  ...
                  %loop assets%            					
        						var assetCompositeNode = new WebFXCheckBoxTreeItem('%value assetCompositeName%%ifvar assetVersion -notempty% (%value assetVersion%)%endif%', "define-repository-components.dsp?projectName=%value ../../../../projectName%&bundleName=%value ../../../../bundleName%&serverAliasName=%value ../../serverAlias%&assetCompositeName=%value assetID%&assetCompositeDisplayName=%value assetCompositeName%&runtimeType=%value ../runtimeType%&regExp=%value /regExp%&regExp2=%value /regExp2%&bundleMode=%value ../../../../bundleMode%", 
        											false, null, webFXTreeConfig.compositeIcon, webFXTreeConfig.compositeIcon,
        											"%value ../../serverAlias -urlencode%$%value ../runtimeType%$%value assetID -urlencode%$%value assetCompositeName -urlencode%$COMPOSITE_INCL", "COMPOSITE_INCL", webFXTreeConfig.linkClass);	 
...
%invoke wm.deployer.gui.UIBundle:getBundleComposites%
	%loop servers%
		%loop composites%
			...
			var p = (document.getElementsByName("%value ../serverAliasName -urlencode%$%value runtimeType%$%value assetID -urlencode%$%value compositeName -urlencode%$COMPOSITE_INCL")).item(0);
			if (p != null) {
