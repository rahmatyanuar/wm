...
CREATE TABLE ConversationScript (
   ...
   ScriptIndex integer,
   ScriptData  blob(100K)
);

ALTER TABLE ConversationScript
       ADD CONSTRAINT pk_ConvScr_ScrName PRIMARY KEY (ScriptName) ;

--********************************************************************

CREATE TABLE Process(
	InstanceID			varchar(128) NOT NULL,
	ParentInstanceID	varchar(128),
...
ALTER TABLE ACTIVITYLOG
       ADD CONSTRAINT fk_ALog_RCoId_Conv FOREIGN KEY (RelatedInstanceID)
                             REFERENCES Process ;
