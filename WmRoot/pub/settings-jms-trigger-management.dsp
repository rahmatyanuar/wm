        ...
        <table width="100%" class="tableView" id="JMS_TABLE">  
          ...
          <tr name="Standard" style="display: table-row;">
            ...
            <td class="subheading2" nowrap>
              State&nbsp;&nbsp;
              <script>
                createForm("htmlform_settings_jms_edit_state", "settings-jms-edit-state.dsp", "POST", "BODY");
                setFormProperty("htmlform_settings_jms_edit_state", "triggerName", "all");
              </script>
              %ifvar webMethods-wM-AdminUI%
                <script>
                  getURL("settings-jms-edit-state.dsp?triggerName=all&setState=check&jmsTriggerTypeReq=0&type=standard&triggerFilterValue=%value triggerFilterValue encode(url)%&aliasFilterValue=%value aliasFilterValue encode(url)%&destinationFilterValue=%value destinationFilterValue encode(url)%&webMethods-wM-AdminUI=true", "javascript:document.htmlform_settings_jms_edit_state.submit();", "edit&nbsp;all");
                </script>
              %else%
                <script>
                  getURL("settings-jms-edit-state.dsp?triggerName=all&setState=check&jmsTriggerTypeReq=0&type=standard&triggerFilterValue=%value triggerFilterValue encode(url)%&aliasFilterValue=%value aliasFilterValue encode(url)%&destinationFilterValue=%value destinationFilterValue encode(url)%", "javascript:document.htmlform_settings_jms_edit_state.submit();", "edit&nbsp;all");
                </script>
              %endif%
            </td>
