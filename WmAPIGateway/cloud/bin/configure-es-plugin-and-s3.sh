...
restartEDS() {
    bash "${relativePath}"/EventDataStore/bin/restart.sh
...
if [[ "$backupToLocationType" == "S3" ]]; then
    ...
    if [[ $environment == "ON-PREMISE" ]]; then
        ...
        restartEDS
        echo -e "API Gateway EventDataStore has been restarted for cloud-aws plugin to be effective"
    fi
    ...
    CONFIGURE_REPO_EXIT_CODE=$?
    echo "CONFIGURE_REPO_EXIT_CODE of configuring Elasticsearch with S3 is $CONFIGURE_REPO_EXIT_CODE"
    echo

    if (($CONFIGURE_REPO_EXIT_CODE > 0)); then
        echo "Configure Elasticsearch with AWS S3 has failed to create backup repository, and hence exiting now.. Please look into this issue.."
        date
        exit 1
    else
        echo "Configuration of Elasticsearch with AWS S3 is successful.."
    fi
